<script>
    ...
    onMount(async () => {
      const compressorModule = await import('./Compressor.svelte');
      CompressorComponent = compressorModule.default;
      isLoaded.set(true);        localStorage.setItem('isLoaded', 'true');

    });
  </script>
  
  {#if $isLoaded}
    <svelte:component this={CompressorComponent} />
  {:else}
  <main
